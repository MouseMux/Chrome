...
// MouseMux: where the injected pointer is, per top-level window, in physical
// pixels.  Code that asks Windows where "the mouse" is - menus re-selecting
// when a submenu opens, tooltips, drag thresholds - then sees the pointer
// driving THAT window rather than the real mouse idling elsewhere, or
// another user's pointer in another window.  A popup resolves to the window
// that owns it.  Defined here, not in the controller, so every binary that
// links ui/display resolves them; declared by extern where used.
namespace content {
struct MouseMuxVirtualCursor {
  HWND root;
  int x;
  int y;
};
namespace {
HWND MouseMuxRootOf(HWND window) {
  return window ? ::GetAncestor(window, GA_ROOTOWNER) : nullptr;
}
}  // namespace

std::vector<MouseMuxVirtualCursor>& MouseMuxVirtualCursors() {
  static base::NoDestructor<std::vector<MouseMuxVirtualCursor>> cursors;
  return *cursors;
}

void MouseMuxVirtualCursorMoved(HWND window, int x, int y) {
  const HWND root = MouseMuxRootOf(window);
  auto& all = MouseMuxVirtualCursors();
  std::erase_if(all, [root](const MouseMuxVirtualCursor& cursor) {
    return cursor.root == root;
  });
  all.push_back({root, x, y});  // back = most recent
}

void MouseMuxVirtualCursorForget(HWND window) {
  const HWND root = MouseMuxRootOf(window);
  std::erase_if(MouseMuxVirtualCursors(),
                [root](const MouseMuxVirtualCursor& cursor) {
                  return cursor.root == root;
                });
}

void MouseMuxVirtualCursorsClear() {
  MouseMuxVirtualCursors().clear();
}

// The injected position for this window, in DIPs: its own if it has one,
// else the most recent of any window.  The fallback matters for menus: a
// popup whose handle does not resolve to the window it serves has no entry
// of its own, and answering with the real mouse there put the synthesized
// move at the real mouse and made the selection flicker again (2026-09-05).
// Only with nothing injected at all does the caller use the real mouse.
bool MouseMuxVirtualCursorAtDIP(HWND window, gfx::Point* out) {
  const auto& all = MouseMuxVirtualCursors();
  if (all.empty()) {
    return false;
  }
  const HWND root = MouseMuxRootOf(window);
  const MouseMuxVirtualCursor* found = &all.back();
  for (const MouseMuxVirtualCursor& cursor : all) {
    if (cursor.root == root) {
      found = &cursor;
      break;
    }
  }
  *out = gfx::ToFlooredPoint(display::win::GetScreenWin()->ScreenToDIPPoint(
      gfx::PointF(gfx::Point(found->x, found->y))));
  return true;
}
}  // namespace content
...
gfx::Point ScreenWin::GetCursorScreenPoint() {
  // No window to ask for here, so the most recently moved injected pointer
  // stands in for "the mouse" while any exists.
  const auto& cursors = content::MouseMuxVirtualCursors();
  if (!cursors.empty()) {
    return gfx::ToFlooredPoint(ScreenToDIPPoint(gfx::PointF(
        gfx::Point(cursors.back().x, cursors.back().y))));
  }
  POINT pt;
  ::GetCursorPos(&pt);
