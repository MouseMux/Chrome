...
#if BUILDFLAG(IS_WIN)
// MouseMux: defined in ui/display/win/screen_win.cc.  The injected pointer
// driving this window, if one is, in DIPs.
namespace content {
...
#if BUILDFLAG(IS_WIN)
  // MouseMux: the mouse moves synthesized here - when a window appears, such
  // as a submenu opening - must land where the injected pointer is, not
  // where the real mouse idles.  Env's last location is fed by every real
  // mouse event in the process, including the idle mouse over the MouseMux
  // dialog, so a menu opened by an injected pointer got a synthesized move
  // at the real mouse and deselected the hovered item (2026-09-04).  Per
  // window: another user's pointer in another window is not this one's.
  if (host_) {
    gfx::Point injected;
    if (content::MouseMuxVirtualCursorAtDIP(host_->GetAcceleratedWidget(),
                                            &injected)) {
      location = injected;
    }
  }
