...
namespace content {
// desktop_window_tree_host_win.cc; see MouseMuxInputController.
...
ui::mojom::DragOperation DesktopDragDropClientWin::StartDragAndDrop(
  ...
  CHECK(!g_is_dragging);
  // MouseMux: no OLE drag for a drag begun by an injected mouse.  DoDragDrop
  // ends only on a real button release, which never comes; the browser
  // process would hang in it (field dump, 2026-09-04).
  if (content::g_mousemux_sdk_button_held) {
