...
void MenuPreTargetHandlerAura::OnKeyEvent(ui::KeyEvent* event) {
  // MouseMux: this handler sits on the process-wide Env, ahead of every
  // window, so while a menu is open it sees every key in the process.  One
  // person at one keyboard never notices: their keys land in the window
  // whose menu is open, because that is the foreground window.  With several
  // users, another user's keys drove and closed a menu they could not see -
  // first their injected keys, dispatched in their own window (2026-09-05
  // 15:40), then the native copies of their keystrokes, which Windows hands
  // to the foreground window, our exempt dialog (15:47).  A key headed for a
  // window other than the menu's own is not the menu's business, whichever
  // way it arrived; it goes on to wherever it was sent.
  if (root_ && event->target()) {
    aura::Window* target = static_cast<aura::Window*>(event->target());
    if (target->GetRootWindow() != root_) {
      return;
    }
  }
