...
LRESULT LegacyRenderWidgetHostHWND::OnMouseActivate(UINT message,
  ...
  // MouseMux: a native click on the page of a blocked window must not
  // activate the window (the click itself is dropped by the parent's hook).
  // Windows asks this child first and never the parent, so the answer has to
  // be given here.  See DesktopWindowTreeHostWin::PreHandleMSG.
  {
    ...
    if (g_mousemux_native_input_blocked ||
        (g_mousemux_blocked_windows && parent &&
         g_mousemux_blocked_windows->count(parent) > 0)) {
