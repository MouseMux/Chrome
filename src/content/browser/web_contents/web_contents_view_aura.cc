...
// Defined in desktop_window_tree_host_win.cc, maintained by
// MouseMuxInputController::NoteSdkButtonState().
...
void WebContentsViewAura::StartDragging(
  ...
  // MouseMux: a drag begun by an injected mouse must not become an OLE drag.
  // DoDragDrop is a modal Win32 loop that ends only on a real mouse-button
  // release, and the injected mouse never produces one: the browser process
  // hangs in that loop (field dump, 2026-09-04).  Tell the renderer the drag
  // is over and carry on; the injected mouse-up arrives as an ordinary event.
  if (g_mousemux_sdk_button_held) {
    web_contents_->SystemDragEnded(source_rwh);
    return;
  }
