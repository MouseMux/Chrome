...
class ToolbarView : public views::AccessiblePaneView,
 ...
 public:
  ...
  // MouseMux: the owner chip before the app menu button - "\u25cf Green" in
  // the user's status colour, or hidden when nobody owns this window.  Set by
  // the MouseMux control dialog whenever ownership changes.
  void SetMouseMuxOwner(const std::u16string& name, SkColor color);
 ...
 private:
  ...
  raw_ptr<views::Label> mousemux_owner_chip_ = nullptr;
