...
void ToolbarView::SetMouseMuxOwner(const std::u16string& name,
                                   SkColor color) {
  if (!mousemux_owner_chip_) {
    return;
  }
  if (name.empty()) {
    mousemux_owner_chip_->SetVisible(false);
  } else {
    mousemux_owner_chip_->SetText(u"\u25cf " + name);
    mousemux_owner_chip_->SetEnabledColor(color);
    mousemux_owner_chip_->SetTooltipText(u"This window belongs to " + name +
                                         u" (MouseMux)");
    mousemux_owner_chip_->SetVisible(true);
  }
  InvalidateLayout();
}
...
void ToolbarView::Init() {
  ...
  // MouseMux: the owner chip, hidden until somebody owns this window.
  {
    auto chip = std::make_unique<views::Label>();
    chip->SetProperty(views::kMarginsKey, gfx::Insets::VH(0, 8));
    chip->SetVisible(false);
    mousemux_owner_chip_ = AddChildView(std::move(chip));
  }
