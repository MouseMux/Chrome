...
#endif
  // MouseMux: "[Green] title" while Green owns this window - the person at
  // the seat sees their own name on it - and "[2] title" while nobody does,
  // the same number the control dialog's users list shows.
  if (base::FeatureList::IsEnabled(features::kMouseMuxIntegration)) {
    std::u16string tag;
    if (aura::Window* native = GetNativeWindow(); native && native->GetHost()) {
      const std::string owner =
          content::MouseMuxInputController::GetInstance()->OwnerNameOfWindow(
              native->GetHost()->GetAcceleratedWidget());
      if (!owner.empty()) {
        tag = base::UTF8ToUTF16(owner);
      }
    }
    if (tag.empty()) {
      tag = base::NumberToString16(
          mouse_mux::WindowNumberForSession(browser_->session_id()));
    }
    title = u"[" + tag + u"] " + title;
  }
  return title;
